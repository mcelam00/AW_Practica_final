<template>
  <v-container>
    ...
    <v-card class="mx-auto" max-width="800">
      <v-img
        src="https://orquideatech.com/wp-content/uploads/2019/07/7-preguntas-que-debes-hacer-en-tus-registros.jpg"
        height="300"
      ></v-img>
      ...
      <v-card-subtitle>
        Responde a las preguntas y gana puntos, por cada acierto se sumará un
        punto
      </v-card-subtitle>
...
export default {
  ...
  methods: {
      ...
      if (this.contestacion != null) {
        if (
          ...
          document.getElementById("puntuacion").innerHTML =
            parseFloat(document.getElementById("puntuacion").innerHTML) + 1;
        } else {
          //Incorrecto
          ...
          document.getElementById("puntuacion").innerHTML =
            parseFloat(document.getElementById("puntuacion").innerHTML) - 0.25;
        }
      ...
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          ...
          document.getElementById("pregunta").innerHTML =
            objetoPregunta.enunciado;
          document.getElementById("solucion").innerHTML =
            objetoPregunta.solucion;
    ...
    guardarPuntos: function() {
      var xhttp = new XMLHttpRequest();
      var url =
        "http://localhost:5000/baseDatos/Preguntas/" +
        document.getElementById("puntuacion").innerHTML;
  ...
  beforeRouteLeave(to, from, next) {
    const answer = window.confirm(
      "¿Seguro que desea abandonar el juego? ¡Ánimo! ¡¡Si ya casi lo tiene!!"
    );
    if (answer) {
      this.guardarPuntos();
      next();
